...
!$omp end parallel do
                ...
                if (.not. ieee_is_finite(rQ) .or. rQ > 1.0e2_dp) then
                   if (n_retry < n_retry_max) then
                      n_retry  = n_retry + 1
                      wloc_irr = max(w_min_irr, wloc_irr*w_shrink_irr)
                      T_prev(:) = T_out(:)
                      cycle
                   end if
                end if
                ...
                if (has_prev) then
                   if (rQ > worsen_tol*rQ_last .and. n_retry < n_retry_max) then
                      n_retry  = n_retry + 1
                      wloc_irr = max(w_min_irr, wloc_irr*w_shrink_irr)

                      ! Important: do NOT accept shadow/Qirr update; retry PDE with smaller w
                      ! Also use current T_out as better initial guess (optional but helps)
                      T_prev(:) = T_out(:)

                      cycle  ! retry same m_irr
                   end if
                end if
