...
program scurve_main
  ...
  namelist /scurve/ r_cgs, r_nd, Sigma_min, Sigma_max, nSigma, &
       Qirr_in, qirr_mode, qirr_file, shadow, outfile
  ...
  ! Set Qirr: constant or read from simulation output
  if (trim(qirr_mode) == 'from_file') then
     call read_qirr_from_disk_file(trim(qirr_file), r_cgs, Qirr_in)
  end if
  ...
  call generate_scurve_at_radius(r_cgs, omegaK_dim(r_cgs / R0), shadow, Qirr_in, &
       Sigma_min, Sigma_max, nSigma, trim(outfile))
...
contains
  ...
  !------------------------------------------------------------
  ! Read Qirr at r_target from disk structure output (disk_t*.dat).
  ...
  subroutine read_qirr_from_disk_file(fname, r_target, Qirr_out)
    ...
    do
       ...
       if (ios /= 0) then
          write(*,'(a,i0,a)') 'ERROR: Cannot parse line ', n, ' in Qirr file'
          stop
       end if
       r_arr(n) = r_cgs
       qirr_arr(n) = qirr
    ...
    if (n == 0) then
       write(*,'(a)') 'ERROR: No data found in Qirr file'
       stop
    end if

    ! Linear interpolation: find i1, i2 such that r_arr(i1) <= r_target <= r_arr(i2)
    if (r_target <= r_arr(1)) then
       Qirr_out = qirr_arr(1)
    else if (r_target >= r_arr(n)) then
       Qirr_out = qirr_arr(n)
    else
       Qirr_out = qirr_arr(1)  ! fallback
       do i = 1, n - 1
    ...
    deallocate(r_arr, qirr_arr)

  end subroutine read_qirr_from_disk_file
